...
const ohMy = () => {
    ...
    axios.get('http://localhost:3000/animals').then((response)=>{
        ...
        for (let i = 0; i < response.data.length; i++) {
            ...
            newPTag.textContent = response.data[i];
...
const repeatMyParam = () => {
    //YOUR CODE HERE
    axios.get(`http://localhost:3000/repeat/tryThis`)
    .then(res => {
...
queryButton.addEventListener('click',()=>{
    axios.get('http://localhost:3000/query-test',{params: {randNumber: 12, randLetter: "A"}}).then((response =>{
        console.log(response.data);
    })).catch((error)=>{
        console.log(error);
    });
...
/*
    ...
    1: Send no queries on the URL -- what happened? 
        Console shows a message saying "you sent an empty query"

    2: Send more than 1 query on the URL -- what happened? 
        Cosole shows an object that contains a message saying "you sent more than 1 query" and a queries object
*/
